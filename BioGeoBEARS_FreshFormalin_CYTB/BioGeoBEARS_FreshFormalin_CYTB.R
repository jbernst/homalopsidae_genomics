...
setwd("D:/Documents/Homalopsidae_Phylogenomics/BioGeoBEARS_FormFresh-CYTB_Test-Manual-Areas/")
...
##############################
#Set up manual ranges_allowed#
states_list<-rcpp_areas_list_to_states_list(areas = c("I", "N", "P", "M", "W", "S", "A", "G"))
states_list
head(states_list)

states_list
length(states_list)
#ranges_list #(not sure what this does? commented out)
#So the above states, IN ORDER, will be labeled as 0,1,2,3,4,5,6,7
#So if we print out states_list, you will see the index number assigned for our states
#(the ones we want are as follows, with their index number in brackets):
# 0       [[2]]
# 1       [[3]]
# 2       [[4]]
# 3       [[5]]
...
# 0123467 [[250]]
#
#Excluding areas that we don't want from the manual ranges_allowed
#If you have just a few ranges you don't want, set FALSE to TRUE on line 427, and then index the 
#TF object to identify which ones you want as FALSE. Here, only a few states are TRUE, so we are doing the opposite
TF = rep(FALSE, length(states_list))
TF
TF[2] = TRUE
TF[3] = TRUE
TF[4] = TRUE
...
length(new_states_list)
BioGeoBEARS_run_object$states_list = new_states_list
...
runslow = TRUE
resfn = "Homalopsidae_DEC_M0_unconstrained_v1.Rdata"
if (runslow)
{
